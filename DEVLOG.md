...
恢復不需按住按鍵的滑鼠左右跟隨。使用鍵盤立即切至鍵盤模式，按住遊戲按鍵時鎖住滑鼠移動；放開後須移動超過 10 CSS 像素才切回滑鼠，避免晃動搶控制。點擊仍可跳躍，暫停／失焦清除輸入。

驗證：新增鍵鼠競爭回歸測試，10 項核心測試、4 項 PWA 測試與 55 項 Chrome 場景測試通過；Chrome 場景以 PointerEvent／KeyboardEvent 驗證角色實際移動方向。

## 2026-09-06 (v2.34.0)

新增四個探索關卡、寶石三星挑戰與個人紀錄；加入彈簧與移動浮台。改用固定 60 Hz 模擬，修正輸入、暫停死亡計時、重生、獎勵房物件還原、Boss 邊界／解鎖、成就與離線版本快取。保留原四世界及無盡模式。

驗證：13 項 Node 測試與 54 項 Chrome 場景測試通過，並實際關閉伺服器驗證離線啟動。Blender 5.2.1 LTS 已安裝，原創練習場景與渲染預覽收錄於 art/blender。完整範圍與限制見 [QA 紀錄](docs/QA-2026-09-06.md)。
